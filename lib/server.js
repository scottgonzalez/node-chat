...
var handlers = [
	...
	{ path: "/cmd", handler: function(channel, request, response) {
		var query = qs.parse(url.parse(request.url).query),
			text = query.text,
			session = channel.sessions[query.id];
		...
		if (!text || !text.length) {
			response.simpleJSON(400, { error: "Must supply a command parameter." });
			return;
		}
		
		session.poke();
		var id = channel.appendMessage(session.nick, "cmd", text);
		response.simpleJSON(200, { id: id });
	} }
];
